...
if selection == "Overview":
    ...
    with col1:
        ...
        if img_intro:
            st.image(img_intro, caption="Market & Traders Locations", use_container_width=True)
        else:
    ...
    # Hotspot Analysis
    st.subheader("Changes in Hotspots")
    
    st.markdown("### Nashik District Hotspot Analysis (Single Kharif)")
    
    # Slider for Year Selection
    hotspot_year = st.select_slider("Select Year for Hotspot View:", options=["2017-18", "2024-25"])
    
    if hotspot_year == "2017-18":
        hotspot_img_name = "nsk_dist_hot_17_18.jpeg"
    else:
        hotspot_img_name = "nsk_dist_hot_24_25.jpeg"
        
    hotspot_img_path = os.path.join(ASSETS_DIR, hotspot_img_name)
    img_hotspot = load_image(hotspot_img_path)
    
    if img_hotspot:
        st.image(img_hotspot, caption=f"Hotspot Analysis - {hotspot_year}", use_container_width=True)
    else:
        st.warning(f"Image not found: {hotspot_img_name}")
...
# --- Taluka Page ---
    ...
    selected_year_label = st.select_slider("Select Year:", options=years)
    ...
    st.markdown("#### Proximity to Mandis")
    fname = f"dist_distrib_{safe_year_str}.jpg"
    img = load_image(os.path.join(taluka_path, fname))
    if img:
        st.image(img, caption=f"Proximity to Mandis ({selected_year_label})", width=600)
    else:
        st.warning(f"File not found: {fname}")
